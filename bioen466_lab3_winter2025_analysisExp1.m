...
file_idnum  = ; %%FILLIN with the numeric value of the file id
...
figure
plot( data([],: ) ) %FILLIN the portion in the brackets to only plot 20 seconds
...
data(:,2) = ; %FILLIN -- use force_voltage_offset to adjust data(:,2) so that it is > 0
...
[stim_times, stim_waveforms] = detectSpikes_filledIn(data(:,1), threshold, FS);
...
[trial_force] = trialAlignData([], align_times, time_before, time_after, FS); %FILLIN. reminder: the force data is the second column in your data set. We only want to align the force data.
...
trial_force_corrected = trial_force - ; %FILLIN to subtract the baseline force for each trial from all time-points in the trial.
...
for iT=1:num_trials
  ...
  idx_rise = find(trial_force_corrected(iT, trial_time>0) > , 1, 'first'); %FILLIN to find when force exceeds our specified threshold.
  ...
  [mx, idx_mx] = max(); %FILLIN

  contraction_amp(iT) = mx;
  contraction_time(iT) = ; %FILLIN. Be sure this is in units of time, not samples.
  ...
  idx_rt = find(trial_force_corrected(iT,[]) < contraction_amp(iT)*.6,1, 'first'); %FILLIN the brackets to search the right piece of the data
  idx_rt = idx_rt + ; %FILLIN to account for searching a subset of the data.
...
%INCLUDE THIS FIGURE IN COMPREHENSION QUESTIONS
figure
plot(trial_time, mean())%FILLIN
hold on
plot([0 0], [0 max([])], 'k--') %plots a dashed line at t=0 to visualize alignment.
